...
export default function CardsPage() {
  ...
  const updateCard = useMutation({
    mutationFn: (data: { cardId: string; front: string; back: string }) =>
      fetch(`/api/decks/${id}/cards/${data.cardId}`, {
        method: "PATCH",
        headers: { "Content-Type": "application/json" },
  ...
  function handleSubmit() {
    if (!stripHtmlTags(front) || !stripHtmlTags(back)) return;

    if (editingCard) {
      updateCard.mutate({ cardId: editingCard.id, front, back });
    } else {
      ...
      <div className="mt-4 flex items-center justify-between">
        ...
        >
          ...
          <Dialog.Portal>
            ...
            <Dialog.Popup className="fixed inset-x-4 top-[5%] max-h-[90vh] overflow-y-auto rounded-lg bg-background p-4 text-foreground outline outline-1 outline-foreground/10 transition-all duration-150 data-[ending-style]:scale-90 data-[ending-style]:opacity-0 data-[starting-style]:scale-90 data-[starting-style]:opacity-0 sm:inset-x-auto sm:top-1/2 sm:left-1/2 sm:-mt-8 sm:w-full sm:max-w-2xl sm:-translate-x-1/2 sm:-translate-y-1/2 sm:p-6">
              <Dialog.Title className="-mt-1.5 mb-1 text-lg font-medium">
                {editingCard ? "Edit card" : "Add a new card"}
              </Dialog.Title>
              <Dialog.Description className="mb-6 text-base text-foreground/50">
                {editingCard
                  ? "Update the front and back of the card."
            ...
            >
              ...
              <div className="ml-3 flex items-center gap-1">
                <button
                  ...
                  className="rounded p-1.5 text-gray-600 hover:bg-foreground/10 hover:text-foreground transition-colors"
